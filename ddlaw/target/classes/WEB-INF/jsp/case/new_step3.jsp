    ...
    $("#done").click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.code==1013){
                	var id=data.caseid;
                    $(".error").fadeIn(500).delay(2000).fadeOut(500).text('新建case成功！');
                    window.location.href="case/"+id+"/todolist";//跳转的地址
                }
                else if(data.code==1012){
                    $(".error").fadeIn(500).delay(2000).fadeOut(500).text('新建case失败，请重试');
                    return false;
                }
                else{}
            }
